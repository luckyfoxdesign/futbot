...
async def set_match_message_id(match_id: int, message_id: int):
    await _db.execute("UPDATE matches SET message_id=? WHERE id=?", (message_id, match_id))
    await _db.commit()


async def get_match(match_id: int) -> Optional[Match]:
    async with _db.execute("SELECT * FROM matches WHERE id=?", (match_id,)) as cur:
        row = await cur.fetchone()
        return _row_to_match(row) if row else None


async def get_open_matches() -> list[Match]:
    async with _db.execute("SELECT * FROM matches WHERE status='open' ORDER BY kickoff_at") as cur:
...
async def update_match_fields(match_id: int, **fields):
    ...
    await _db.execute(f"UPDATE matches SET {sets} WHERE id=?", vals)
    await _db.commit()
...
async def mark_reminder_sent(match_id: int, field: str, now_iso: str):
    ...
    await _db.execute(f"UPDATE matches SET {field}=? WHERE id=?", (now_iso, match_id))
    await _db.commit()
...
async def execute_immediate(coro):
    """Run coroutine after acquiring BEGIN IMMEDIATE; caller must commit/rollback."""
